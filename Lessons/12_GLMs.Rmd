...
getwd()
library(tidyverse)
options(scipen = 4)#prevent e, 4 digital after 0 will be shown
...
# Correlation
r<-cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
R2=r*r
```
Question: How would you report the results of this test (overall findings and report of statistical output)?

>  At greater depths, irridation decrease. For each 1m increase in depth, irridance decrease by 95.89 units(Linear regression, R2=0.31, df=15,449, p<0.001).
>Irrdiance decreases significantly with increasing depth(Linear regression, R2=0.31, df=15,449, p<0.001)
>Depth accounts for 31% of variance in lake irradiance(Linear regression, R2=0.31, df=15,449, p<0.001).
...
par(mfrow = c(2,2), mar=c(1,1,1,1))#mar for printing better, do not need to include the plot in the report.
plot(irradiance.regression)
par(mfrow = c(1,1))##restore plot one by one
```
...
#data exits the exponential relationship, consider log tranfer data
```
...
##filter out the 0 before log transformation, adjusted R2 give you penality of low sample size, which is better.
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() +#alternatively is to log the y in aes.
  geom_point() 
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha=0.3,pch=1,color="pink") +
    scale_y_log10() +
  labs(alpha=0.5,x="Depth(m)", y="Radiance")+
    geom_smooth(method = 'lm', alpha=0.5,se = FALSE, color = "black")
print(irradiancebydepth2)
#se=FALSE is removin the standard error.
...
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth, 
         method = "spearman", exact = FALSE)#use this test, when it does not meet the linear assumption. exact is the exact p value
```
...
corrplot.mixed(PeterPaulCorr, upper = "ellipse")#this is better with value.
```
...
```{r}
#AIC value is not comparable across different contaext.
Paul.naomit <- PeterPaul.chem.nutrients %>%
  filter(lakename == "Paul Lake") %>%
  na.omit()

TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)#pick up the high nutrients based on the corelation test.
step(TPAIC)
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)

summary(TPmodel)#tn is not significnant by itself but it does count significant varience of the data set.

```
Total p is significnatly correlated to the temperature, dissolved oxygen and 