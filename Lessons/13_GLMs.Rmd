...
Tukey test check main effect or interaction, never both.
...
biomarker <- read.csv("./Data/Raw/biomarker1.csv")
...
# interaction makes the two lines  of different lake have different slope.
...
# Make the graph attractive, how do you know the lm consider the interaction?
```
...
```{r}
TempTest.mixed <- lme(data = NTL.summertemp,#lme is linear mixing effect.
                     temperature_C ~ year4, 
                     random = ~1|lakename)#let the lake to be random effect
summary(TempTest.mixed)
rsquared(TempTest.mixed)#marginal is the fixed effect, conditional R2 is after adding random effect

# Compare the random effects model with the fixed effects model
TempTest.fixed <- gls(data = NTL.summertemp,
                      temperature_C ~ year4)#gls gives a slightly different output???
summary(TempTest.fixed)

anova(TempTest.mixed, TempTest.fixed)
# The lower the AIC, the better.
# The p-value tells us whether those models have a significantly different fit, the null hypothesis is that the models do not have significantly different.
#AIC is only useful when you compare models together in a AIC()
...
print(NTL.tempmodel)
#using geom smooth is dangerous, always need to know what do you want to graph
```
