...
vector1[3] # pull out the oject at the 3rd place.

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # matix is a function
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow count from left to right first, the defalut is to count from top to the bottom first.
matrix2
...
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #row first,column second.
matrix3
matrix1[4, ] #
matrix1[ , 3] #
matrix1[c(12, 14)] # find the number 
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
matrix3[c(12, 14)]
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # ;can reduce lines
...
> Answer: it gives the option about the potential names
...
```{r}
t1 <- c(60:69);t2 <- c(70:79)
tem <- data.frame(t1,t2)  
names(tem) = c(" Durham low"," Durham high")
tem1
```
...
c1 <-(t1 − 32) * 5/9; c2 <-(t1 − 32) * 5/9
tem1 <- data.frame(t1,c1,t2,c2,t3,c3,t4,c4)  
names(tem1) = c(" Durham low F", " Durham low C"," Durham high F"," Durham high C", )
tem1
```
