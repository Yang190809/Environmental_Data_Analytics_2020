...
```{r, message = FALSE}
#load the package without the message
# Load package
library(tidyverse)
```
...
#one dot is going in one level, tow dots is going out one level.
...
#in this case,you have to rename all of it, or else use [] to specify which column
colnames(USGS.flow.data)[1] <- c("agency_cd")
str(USGS.flow.data)#strucure of the data frame???
dim(USGS.flow.data)#dimension:raws and column
length(USGS.flow.data)#give you number of column
nrow(USGS.flow.data)

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)]#define raws,column
...
```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data)#take any data raw that has any NA.
dim(USGS.flow.data)
...
today <- Sys.Date()#base your computer dat
today
...
#tells R what is the format of the data
```
