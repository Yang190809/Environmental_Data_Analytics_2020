...
# Set theme
mytheme <- theme_classic(base_size = 14) + #base size of font
...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean)#na.omit and na_exclude are the same.
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365)#365 generate daily data, [8] choosespecific column.
```
...
#stl only works with time series data frame, 

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed)#trend is movign average.the bar are the same in the scale. the bar scale of the trend and seasoal are much smaller than the data, which means the data cannot be explained by the seasonal and trend.
#one season here is the cycle of 365 (a year)
...
EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
                      ...
                      Date = EnoDischarge.late$datetime)#adding two columns
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))
#stl can produce negative value 
...
```{r}
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000)#subsample 5000 for the shapiro test
EnoDischarge.late.subsample <- sample_n(EnoDischarge.late, 5000)

#t test assumption are the normality and equal variance
...
#we cannot use t test, use wilcox.test instead
wilcox.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean)
#significant difference between discharge from two period.
...
>The discharge from early period are significantly higher than the late period (Wilcoxon test, p value<0.001) .
...
> aggregation: using many data points to generate one data points.

> subsampling: random 

> interpolation: fill up gaps
...
#for the gap, using interpolation might not be good, as the complete data has a lower curves
NCPM2.5 <-
...
# na.approx function fills in NAs with a linear interpolation (using mean)
...
#these interpolation assume the time points are equal spaced
...
* **Mann-Kendall**: no seasonality, non-parametric, no temporal autocorrelation, missing data allowed. Function: `mk.test` (package: trend),assume normal distribution.
...
EnoDischarge.late.monthly.ts <- ts(EnoDischarge.late.monthly$Discharge, frequency = 12, 
                        start = c(1985, 10, 1), end = c(2019, 12, 1))#the cycle is a year, with a frequence of 12 month,time series does not show columns in tenvironment.


# Run SMK test
EnoDischarge.late.trend <- smk.test(EnoDischarge.late.monthly.ts)#put itme series inside

# Inspect results
EnoDischarge.late.trend #pa value is compute from the z value, negative z means decresing trend, p value big means no significance
summary(EnoDischarge.late.trend)
# no significnat difference between the same month of different year
...
> The discharge of Eno river does not show significant change in last 30 years(SMK, Z=-0.1397 , p=0.8889)
Even though one or more months have significant difference, the overall test does not necessary turn out to be signifcant. This test does not have freedom.ALways report the z value.
