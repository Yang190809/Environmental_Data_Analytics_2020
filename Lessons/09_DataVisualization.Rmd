...
# do as.date everytime you upload a data
...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()#theme do not adjust legend
print(O3plot4)
```
...
theme_set(mytheme)# it can apply all the plots that underneath
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
#AQI is raltive air quality compared to the standard
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) +# horizontal line, line type
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) +# point transparence
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +# specify the location using the x, y, horizontal ajust
  geom_text(x = as.Date("2020-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") +#adjust to the right for unit of 1
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") +# lower case month
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) +xlab("tn(ug/l")+ylab("tp(ug/l)")
