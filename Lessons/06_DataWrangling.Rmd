...
Ecosystem health, eurafication and alagae boom. 
 Help to manage and imply to policy and regulation
 Monitor the climate change
> Add notes here: 

```{r, message = FALSE}
getwd()
library(tidyverse)
#install.packages(lubridate)#always comment out the package installing
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv")

#. means go in the parent folder, .. means go out the parent folder,../
...
summary(NTL.phys.data$daynum)#no sample during frozen season
...
year99 <- filter(NTL.phys.data, year4 == 1999)
year99

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
Tu.year99 <- filter(NTL.phys.data, year4 > 1999 | year4 < 1999 &
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)#default is increase
...
tem.dat <- arrange(NTL.phys.data, desc(temperature_C))
```
### Select
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C)#filter is for horizontal and select is usually for column
no.lakename <- select(NTL.phys.data, -lakename, sampledate:temperature_C)#it only exclude the lakename here
```
### Mutate
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%#select the data sect then
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #select rows
  select(lakename, sampledate:temperature_C) %>%#select column
