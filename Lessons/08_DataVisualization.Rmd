...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()#this way can not show in environment.

O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot)

# Fix this code: color should be outside the aes
...
PMplot <- 
  ...
  geom_point()
print(PMplot)#year is interger, use as.factor

# Separate plot with facets
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) +
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3)#nrow is to indicte the row of images.
print(PMplot.faceted)# sites were printed in alphabatic order.
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #allow us facet among two variables
#Site.Name%in% c("","","")
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line()+ #continous
  geom_point() #add extra layer of points
print(PMplot.line)
...
tem.plot <-
  ...
  geom_point(aes(x = daynum, y = temperature_C, color = year4)) +#put aes in point is better to have different layers
...
#when you have two places and different depth, youshould not use the line (time series)
```
...
lightvsDOfixed <- 
  ...
  xlim(0, 250) +
  ylim(0, 20)
print(lightvsDOfixed)#deep places have very low O2
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) +#linear model, the shadow is the confidence of the model,se=false can remove the the confidence interval.
  geom_abline(aes(slope = 16, intercept = 0))#16:1 is a standard ratio of N to P.
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"?, fill is filling the box, you can not see the color when it is near zero using fill.
print(Nutrientplot3)
...
#same value get spread horizontally
print(Nutrientplot4)
...
library(ggridges)
Nutrientplot6 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(y = concentration, x = nutrient)) +
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #
print(Nutrientplot6)

#do not use bar plot unless it is count data, it does not consider
