...
```{r, message = FALSE}
#load the plyr then dplyr later, so we can mask out the plyr 
getwd()
library(plyr)
library(tidyverse)
library(lubridate)
...
#after filters use the droplevel can remove all the zero
summary(NTL.nutrient.data.PeterPaul$lakename)
...
NTL.nutrient.data.subset <- 
  NTL.nutrient.data.PeterPaul %>% #
  select(lakename=irradianceDeck) %>% #
  select(lakename, year4, daynum, month, sampledate, depth:po4) 
```
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, "nutrient", "concentration", tn_ug:po4)#one column name is nutrient, filling with the name of the nutrients, another column is concentration, it will display the concentration value
...
#separate create new column but not keep the original column
...
Litter <- LitterFiles %>%
  ldply(read.csv)
#only use for the data file with the same column, for the different column, use the join function
```
...
#specify the column to join(those column exist in both dataset), the final data set will not repeat these columns.
...
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>%#select the file
  filter(depth == 0) %>%
  group_by(lakename, month) %>%
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>%#math is not allowed when there is na 
  summarise(meantemp = mean(temperature_C), #create new column
            sdtemp = sd(temperature_C), 
            meanTN = mean(tn_ug), 
            sdTN = sd(tn_ug), 
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#this cundtion need to load plyer and load deplyer
