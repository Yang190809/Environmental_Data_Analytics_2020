...
#all 11 ID are significantly different 
...
# Extract groupings for pairwise relationships, predict factor is ID
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
Litter.Totals.groups
#if any letter match , it meas not significantly different
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  ...
  ylim(0, 85)
#vjust just vertically, the order of x is default to increase ID, define the location of "abcd" above the maximum point,
...
> total litter biomass differed significantly among sites at the Niwot Ridge LTER (figure 1: ANOVA, F=4.813(11,198), p value=)
#p value is useless without know the type of test. The further the Fvalue different than 0, the higher possibility of sinigicantly low p value
...
#result show that interaction is significant
# Format as lm
