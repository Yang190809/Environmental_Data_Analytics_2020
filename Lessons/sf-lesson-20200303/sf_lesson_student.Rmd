...
here("data", "tabular_data", "conterm_basinid.txt")# go to data, tabular, find conterm and generate the file path to it.
...
```{r basic.sf.plotting}
# basic plot
#leave ggplot() open, because we want to add many layers, do the boundary first
...
ggplot() +
  geom_sf(data = nc_state_bounds_geom,color="brown") + 
   geom_sf(data = nc_basins_nf_seplains,fill="blue", alpha=0.25) + 
   geom_sf(data = nc_gages) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines
  theme_bw() # add this to make the background white and not grey
dev.off()
...
nc_bbox <- nc_state_bounds_geom %>%
  ...
  st_transform(4326) %>% # WGS84 (for lat and long, projecting back to longitude and latitude)
  st_bbox()
nc_bbox # find the coordinates of the box
...
#loading the tile based on the zoom
class(nc_basemap)
st_crs(nc_basemap)
...
st_write(nc_state_bounds_geom, here("outputs", "nc_boundary.shp"))#save the shp file
```
