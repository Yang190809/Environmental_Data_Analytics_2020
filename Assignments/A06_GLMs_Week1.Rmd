...
> Answer: I will choose the two-way ANOVA with interaction because it involves two category factors including lake and season. Using interaction effect are more accurent as it including main effect and the interaction effection between lake and season.
...
```{r}
#5
shapiro.test(lake$tn_ug)
tn.anova <- aov(data = lake, tn_ug ~ month*lakename)
summary(tn.anova)
tn.lm<-lm(data = lake, tn_ug ~ month*lakename)
summary(tn.lm)

#6
shapiro.test(lake$tp_ug)
tp.anova <- aov(data = lake, tp_ug ~ month*lakename)
summary(tn.anova)
tn.lm<-lm(data = lake, tn_ug ~ month*lakename)
summary(tn.lm)
