...
lake.july <- 
lake%>%
filter(daynum >180 & daynum <211)%>%
select(lake, lakename, year4, daynum, depth, temperature_C)%>%
na.omit()


#4

cor.lake <- cor(lake.july)
cor.plot<- corrplot.mixed(cor.lake, upper = "ellipse")
...
#8
lake.plot <-
ggplot(lake.july, aes(x = depth, y = temperature_C)) +
...
rsquared(litter.mixed)#marginal is the fixed effect, conditional R2 is after adding random effect
```
...
> Answer: The mixed effects model is a better model than the fixed effects model because it has higher R2 value, which means the model has higher power in expalinin gthe varience of the data.