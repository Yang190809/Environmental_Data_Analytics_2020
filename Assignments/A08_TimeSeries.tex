...
\KeywordTok{select}\NormalTok{(Date, Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration,DAILY_AQI_VALUE)}
...
\CommentTok{# 6}
\NormalTok{GaringerOzone<-}\StringTok{ }\KeywordTok{merge}\NormalTok{(}\DataTypeTok{x=}\NormalTok{Days, }\DataTypeTok{y=}\NormalTok{Ozone, }\DataTypeTok{by=}\StringTok{"Date"}\NormalTok{, }\DataTypeTok{all.x=}\OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha=}\FloatTok{0.2}\NormalTok{,}\DataTypeTok{color=}\StringTok{"darkgreen"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Date"}\NormalTok{, }\DataTypeTok{y =} \KeywordTok{expression}\NormalTok{(}\StringTok{"Ozone Concentration(ppm)"}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{ylim}\NormalTok{(}\DecValTok{0}\NormalTok{,}\FloatTok{0.12}\NormalTok{)}\OperatorTok{+}
...
\KeywordTok{print}\NormalTok{(ozone.plot)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: The ozone concentration from day to day are related to each
other, which means the ozone concentration from any date are ralated to
the data from two neighbor date. Therefore, using the linear
interpolation can consider the both data around the missing data while
interpolating. Spline interpolation might exaggerate the actual
concentration.
\end{quote}
...
\begin{quote}
Answer: Our data have seasonality and do not have temporal correlation,
which fits the requirements of Seasonal Mann-Kendall. Normal
Mann-Kendall does not consider the seasonality and modified Mann-Kendall
...
\NormalTok{GaringerOzone}\OperatorTok{%>%}
...
\KeywordTok{summarize}\NormalTok{(}\DataTypeTok{mean.ozone =} \KeywordTok{mean}\NormalTok{(Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration))}
...
\CommentTok{# 10}
\NormalTok{GaringerOzone.monthly.ts<-}\KeywordTok{ts}\NormalTok{(GaringerOzone.monthly}\OperatorTok{$}\NormalTok{mean.ozone, }\DataTypeTok{frequency =} \DecValTok{12}\NormalTok{, }
...
\begin{Highlighting}[]
\CommentTok{# 13}
\NormalTok{GaringerOzone.plot <-}
\KeywordTok{ggplot}\NormalTok{(GaringerOzone.monthly, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ Date, }\DataTypeTok{y =}\NormalTok{ mean.ozone)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha=}\FloatTok{0.5}\NormalTok{, }\DataTypeTok{color=}\NormalTok{(}\StringTok{"darkgreen"}\NormalTok{)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_line}\NormalTok{()}\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Date"}\NormalTok{, }\DataTypeTok{y =} \KeywordTok{expression}\NormalTok{(}\StringTok{"Ozone Concentration(ppm)"}\NormalTok{)) }
...
\begin{quote}
Answer: The ozone concentraion at Garinger school does not show
significant change from 2010 to 2019(SMK, Z=-1.963 , p=0.04965)
\end{quote}
